...
public class QueryAlgebraFactory {
...
//	private Map<String, Var> varMap = new HashMap<String, Var>();
	...
	private QueryAlgebraFactory(Graph graph, Node queryRootNode, Map<Node, Var> parentVarMap) {
		this.graph = graph;
...
//			ElementTriplesBlock elementTriplesBlock = new ElementTriplesBlock();
			...
			SortedSet<ElementContext> elementContexts = new TreeSet<QueryAlgebraFactory.ElementContext>();
...
//		return new QueryFactory(graph).toQuery(queryRootNode);
		return new QueryAlgebraFactory(graph, queryRootNode, new HashMap<Node, Var>()).toQuery();
	}
...
//					}
//				})
				.mapWith(new Function<Triple, Triple>() {
...
//		return new QueryFactory(graph).toQuery(queryRootNode);
		return new QueryAlgebraFactory(graph, queryRootNode, parentVarMap).toQuery();
	}
	
	public static Update toUpdate(Graph graph, Node queryRootNode) {
//		return new QueryFactory(graph).toQuery(queryRootNode);
		return new QueryAlgebraFactory(graph, queryRootNode, new HashMap<Node, Var>()).toUpdate();
	}
