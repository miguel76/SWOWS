 ...
 */
public class RecursiveDatasetProducer extends DatasetProducer {
...
//	private BuildingGraph localGraph = new BuildingGraph(Graph.emptyGraph);
	 ...
	 */
	public void attacheToValue(NotifyingProducer prod) {
	 ...
	 * @param prod the producer
	 */
	public void attacheToReset(NotifyingProducer prod) {
		connectedProducer = prod;
		prod.registerListener(
				new DatasetProducerListener() {
					public void notifyDatasetCreation(DynamicDataset inputDataset, DynamicDataset dataset) {
						attacheResetGraph(inputDataset, dataset.getDefaultGraph());
						Iterator<Node> graphNames = dataset.listGraphNodes();
						while (graphNames.hasNext()) {
							Node graphName = graphNames.next();
							attacheResetGraph(inputDataset, dataset.getGraph(graphName));
						}
					}
				});
	}
	 ...
	 * @param prod the producer
	 */
	private void attacheResetGraph(DynamicDataset inputDataset, DynamicGraph resetGraph) {
		getLocalDefaultGraph(inputDataset).addAsResetGraph(resetGraph);
		Map<Node,RecursionGraph> inputLocalNamedGraphs = localNamedGraphs.get(inputDataset);
		if ( inputLocalNamedGraphs != null ) {
			for (RecursionGraph graph : inputLocalNamedGraphs.values()) {
				graph.addAsResetGraph(resetGraph);
			}
		}
	}
