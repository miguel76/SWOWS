 ...
 * SWOWS (as well as TUIO Java backend) is free software: you can redistribute it and/or modify
...
/**
 * This constructor creates a client that listens to the default TUIO port 3333 with provided refresh mode
 *
 * @param  autoRefresh  if set to true this object will regularly send refresh to listeners even if no new messages have been received
 */
public TuioClient(boolean autoRefresh) {
	this(DEFAULT_PORT, autoRefresh);
}
...
/**
 * This constructor creates a client that listens to the provided port with provided refresh mode
 *
 * @param  port  the listening port number
 * @param  autoRefresh  if set to true this object will regularly send refresh to listeners even if no new messages have been received
 */
public TuioClient(int port, boolean autoRefresh) {
	this.port = port;
 ...
 */
public synchronized void acceptMessage(Date date, OSCMessage message) {
	...
	if (address.equals("/tuio/2Dobj")) {
		...
		if (command.equals("set")) {
			...
			if (objectList.get(s_id) == null) {
				...
				if (tobj==null) return;
				if ( 	(tobj.getPosition().getX() != xpos)
						|| (tobj.getPosition().getY() != ypos)
						|| (tobj.getAngle() != angle)
		...
		if (command.equals("set")) {
			...
			if (cursorList.get(s_id) == null) {
				...
				if ( 	(tcur.getPosition().getX() != xpos)
						|| (tcur.getPosition().getY() != ypos)
